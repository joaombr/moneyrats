...
async def entrar_grupo(request: Request, codigo: str = Form(...), db: Session = Depends(get_db)):
    # 1. Recupera o e-mail do cookie (Sessão)
    email_logado = request.cookies.get("user_email")
    codigo_formatado = codigo.strip().lower()
    
    if not email_logado:
        # Se não houver cookie, manda o usuário logar de novo
        return RedirectResponse(url="/?erro=sessao_expirada", status_code=303)

    # 2. Busca o grupo (usamos .upper() para garantir que case-insensitive funcione)
    # O código no banco é gerado pelo uuid e costuma ser salvo como string
